...
```{r, message=FALSE}
library(scran)
keep <- !(isOutlier(mouse.sums, n=3, log=TRUE, type="lower") | 
          isOutlier(ercc.sums, n=3, log=TRUE, type="lower") | 
          isOutlier(sirv.sums, n=3, log=TRUE, type="lower"))
...
diagnoseVariance(spike.data[,keep], spike.data$samples$ercc.first[keep], design[keep,])
diagnoseVariance(spike.data[,keep], spike.data$samples$induced[keep], design[keep,])
```
