...
<!--
The evaluation of some code blocks depends on whether
they are executed locally (i.e., on the author's machine)
or using the public data. Results are, fortunately, the same.
-->
...
" } else { "
We load in the annotation.
"}
`

```{r, echo=.spike.local, eval=.spike.local}
...
```{r, echo=!.spike.local, eval=!.spike.local} 
anno <- read.table("sdrf.tsv", header=TRUE, sep="\t", fill=TRUE, 
    check.names=FALSE, stringsAsFactors=FALSE, comment="")
...
" } else { "
Pulling out the control type for each well.
"}
`
...
```{r, echo=!.spike.local, eval=!.spike.local}
conanno <- anno[,"Characteristics[single cell well quality]"]
control.type <- character(nrow(anno))
control.type[conanno=="50 cells"] <- "+"
control.type[conanno=="no cell"] <- "-"
```
...
```{r, echo=.spike.local, eval=.spike.local, message=FALSE}
library(simpaler)
