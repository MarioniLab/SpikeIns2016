...
The first step of the experiment involves adding spike-in populations to each well of a microtiter plate.
...
$\phi_{is}$ & The efficiency of cDNA generation in well $i$ for all transcripts of population $s$. \\
...
$\phi_{is}$ is a random variable that represents well-specific biases for each spike-in population.
...
The other exception is that $\phi_{is}$ may not be independent between $s$, as capture efficiencies of different populations may be affected by the same factors in each well.
