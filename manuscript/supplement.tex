...
Specifically,
\[
    L_i = D_i \left[ \sum_{s \in \{1, 2\}} \left( l_s V_{is} R_{is} \sum_{t_s} r_{t_s} c_{t_s} \right) \revised{+ l_0 R_{i0} \sum_g r_g N_{ig}} \right]^{-1}
\]
where $D_i$ is \revised{a random variable representing} the total sequencing depth for well $i$ (in reads);
\revised{$N_{ig}$ is a random variable specifying the number of transcripts for each endogenous gene $g$ in $i$;
...
Any correlation between $L_i$ and the other terms would be negligible compared to the biological variance of expression\revised{, i.e., $\variance{(N_{ig})}$}.
...
For \revised{each plate in} each data set (416B or TSC), we extracted the spike-in count data for the premixed wells.
...
\revised{We fitted a GLM to the counts for each transcript using a design matrix with a one-way layout.
...
By forcing the offsets \revised{of all wells} to be equal, \revised{we removed differences in the log-ratios between wells due to cell-specific capture efficiency and sequencing depth.
...
We also downscaled the fitted values to determine the effect of \revised{reducing spike-in coverage on the sampling noise.
...
For DEG detection with edgeR \revised{v3.18.1}, a NB GLM was fitted to the counts for each gene \citep{mccarthy2012differential} using a suitable design matrix.
...
For MAST \revised{v1.2.1}, an effective library size was defined by multiplying the total spike-in count by a constant value $C$ for each library.
...
\revised{This was parameterized as an additive model for all experimental factors}, with the proportion of genes with non-zero counts in each library included as a covariate \citep{finak2015mast}.
...
The rest of the method was implemented as originally described, using the technicalCV2 function in the scran package \revised{(v1.4.4)} with min.bio.disp set to zero.
...
\begin{figure}[btp]
    \begin{center}
        \begin{subfigure}{0.49\textwidth}
            \includegraphics[width=\textwidth,page=1]{../simulations/depth/depth_effect.pdf}
            \caption{}
        \end{subfigure}
        \begin{subfigure}{0.49\textwidth}
            \includegraphics[width=\textwidth,page=2]{../simulations/depth/depth_effect.pdf}
            \caption{}
        \end{subfigure}
    \end{center}
    \caption{Effect of spike-in coverage on the variance of the log-ratios of the ERCC and SIRV total counts, using simulations based on the (a) 416B or (b) TSC data sets.
        ...
        \revised{The last point of each line represents the variance at 100\% coverage on the corresponding plate.
