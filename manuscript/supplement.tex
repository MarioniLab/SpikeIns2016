...
This suggests that most of the variance in the premixed experiments is due to sampling noise rather than due to variable differences in behaviour, i.e., $\variance(F_i)$.
In addition, we also scaled the fitted values to reflect reduced coverage of the spike-in transcripts, e.g., due to addition of spike-in RNA at lower concentrations.
This did not affect the variance of the log-ratios in either data set, even when only 10\% of the original coverage was used.
These results suggest that spike-in normalization is robust to the use of low amounts of spike-in RNA.
