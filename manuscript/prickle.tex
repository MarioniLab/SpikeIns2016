...
This means that spike-in normalization can be applied in situations where large-scale changes in expression (e.g., related to changes in total RNA content) are expected and of interest \cite{lun2016stepbystep,nestorowa2016single}.
...
Each well also contained a single mouse cell -- a mouse 416B myeloid progenitor cell or trophoblast stem cell (TSC) -- to mimic real experimental conditions.
The resulting pool of endogenous/spike-in RNA in each well was used to generate a cDNA library, using a modified version of the Smart-seq2 protocol.
This process was repeated for multiple wells and high-throughput sequencing was performed on all libraries.
...
However, the relevant quantity is the variability in the difference $P_{is}$ between a synthetic spike-in set and endogenous RNA, as this affects the accuracy of normalization.
...
The murine multipotent myeloid progenitor cell line 416B \cite{dexter1979isolation} was maintained in RPMI medium, supplemented with 10\% fetal calf serum and antibiotics.
\textcolor{red}{Need details of oncogene and its induction.}

Murine TSCs were kindly provided by Dr. Jennifer Nichols and cultured on mouse embryonic fibroblast (MEF) feeders with TSC culturing medium (a combination of 70\% MEF conditioned media (R\&D systems) and 30\% RPMI 1640, supplemented with 20\% FBS, 2 mM L-glutamine, 1 mM sodium pyruvate, 100 \textmu{}M \textbeta{}-mercaptoethanol, 25 ng/mL human recombinant FGF4 (R\&D systems) and 1 \textmu{}g/mL heparin (Tocris Bioscience)). 
...
Sequencing of the 416B libraries was performed at the Genomics Core facility of the Cancer Research UK Cambridge Institute.
The first batch of libraries was sequenced with an Illumina HiSeq 2500 machine generating 125 bp single-end reads, while the second batch was sequenced with an Illumina HiSeq 4000 machine generating 50 bp single-end reads.
Sequencing of the TSC libraries was performed at the Single Cell Genomics Core facility at the Wellcome Trust Sanger Institute.
Both batches were sequenced using an Illumina HiSeq 4000 machine generating 75 bp paired-end reads.
...
\section{Data access}
Data are available on ArrayExpress under the accession E-MAT-5522.
...
\section{Funding}
This work was supported by core funding from the CRUK (award no. A17197) and core funding from EMBL to JCM.
LHV was supported by an EMBL Interdisciplinary Postdoctoral fellowship.
Work in the G\"ottgens group was supported by CRUK, Bloodwise, NIDDK, the Leukemia and Lymphoma Society and core infrastructure grants from the Wellcome Trust and MRC to the Cambridge Stem Cell Institute.
