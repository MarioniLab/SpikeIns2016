...
\revised{In general, scaling normalization for RNA-seq data is performed by dividing all counts in each library by a library-specific constant, known as the ``size factor''.
...
\revised{These results show that the variance of volume addition is minor compared to other technical and biological sources of variability in a scRNA-seq experiment, and thus is unlikely to have a major effect on spike-in normalization.}
...
By comparison, the size factors routinely vary by more than 30\% across cells (Supplemental Figure~\suppfigsizevar{}).
This suggests that the loss of precision due to noise is minor and can be ignored during spike-in normalization.
}
