...
\]
where $R_i = \log(R_{i1}/R_{i2})$ and represents the fold-difference in the average capture efficiency between the two sets (i.e., the difference in behaviour of the spike-ins).
...
\]
It should be stressed that this variance estimate is relevant to all experiments using the same protocol for spike-in addition, even if the identity or concentration of the spike-in set are different.

The variance of $R_i$ is also relevant as it determines the effect of differences in behaviour between different RNA populations.
Even when the capture efficiency differs between spike-in and endogenous transcripts, spike-in normalization may still be accurate, \textit{provided that the fold difference in capture efficiency is the same in all wells}.
This is because any consistent difference in efficiency would scale the counts in all libraries to the same extent, thus effectively cancelling out upon scaling normalization.
However, if the relative efficiency varies across cells, the accuracy of spike-in normalization would be compromised.
The estimated variance of $\theta'_i$ provides an upper bound for the variance of $R_i$, i.e., the difference in efficiency between the ERCC and SIRV sets.
These two sets of transcripts have substantial differences in their biophysical properties (Supplementary Figure XXX), similar to the differences between synthetic and endogenous transcripts. 
This suggests that $\variance(R_i)$ can provide a rough estimate of the variance in relative capture efficiency between the spike-in and cellular RNA.
