...
\revised{Specifically, we estimated the variance due to sampling noise to be 0.005-0.013 (using the original spike-in coverage for each plate), compared to estimated values of 0.010-0.015 for $\variance(\theta^*_i)$ in Figure~\ref{fig:varestimates}.
...
In typical experiments, spike-in transcripts take up 5-10\% of the library size for each cell (\revised{50,000-100,000} reads in our data).
...
\revised{For each data set, data from both plates were combined for a single round of HVG detection.}
